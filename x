...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = Forbidden.PointLock.Reach_Visibility
            Circle.Radius = Forbidden.PointLock.Reach
...
local function GetTarget()
    if Forbidden.Silent.mode == "target" then
        return TargetPlayer  
    elseif Forbidden.Silent.mode == "normal" then
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == Forbidden.Target.Keybind:lower() then
        if Forbidden.PointLock.Enabled then
            if IsTargeting then
    ...
    elseif Key:lower() == Forbidden.Target.UntargetKeybind:lower() then
        IsTargeting = false
        TargetPlayer = nil  
    end
end)
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if Forbidden.PointLock.Aligner then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * Forbidden.PointLock.VelocityCompensation)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * Forbidden.PointLock.VelocityCompensation)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if Forbidden.PointLock.StabilizationEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, Forbidden.PointLock.Stabilization)
                else
                    Camera.CFrame = DesiredCFrame
                end
            end

            if Forbidden.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if Forbidden.Silent.mode == "target" then
                    local closestPoint
                    if Forbidden.Silent.TargetMode == "OptimalTargetPoint" then
                        closestPoint = GetOptimalTargetPoint(TargetPlayer.Character)
                    elseif Forbidden.Silent.TargetMode == "Closest Point" then
                        closestPoint = GetClosestPoint(TargetPlayer.Character)
                    elseif Forbidden.Silent.TargetMode == "BasicTargeting" then
                        closestPoint = GetClosestHitPoint(TargetPlayer.Character)
                    end

                    local velocity = GetVelocity(TargetPlayer, "Head")  
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * Forbidden.Silent.Prediction)
                elseif Forbidden.Silent.mode == "normal" then
...
local function getDynamicFov(targetPosition)
    ...
    if distance <= getgenv().Forbidden.Ranges.CloseDistance then
        return getgenv().Forbidden.Silent.BoudingSizes.CloseFovRadius
    elseif distance <= getgenv().Forbidden.Ranges.MidDistance then
        return getgenv().Forbidden.Silent.BoudingSizes.MidFovRadius
    elseif distance <= getgenv().Forbidden.Ranges.FarDistance then
        return getgenv().Forbidden.Silent.BoudingSizes.FarFovRadius
    else
        return getgenv().Forbidden.Silent.BoudingSizes.FarFovRadius  
    end
end
...
local function updateFov(TargetPlayer)
    local settings = getgenv().Forbidden.Silent.BoudingSizes
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    if not getgenv().Forbidden.PointLock.Enabled or not IsTargeting or not TargetPlayer then
        return false
    end
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if velocity.Y < -30 and getgenv().Forbidden.TargetControl.Aligner then
            getgenv().Forbidden.Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Forbidden.TargetControl.Aligner then
            ...
            getgenv().Forbidden.Silent.Prediction = getgenv().Forbidden.Silent.Prediction -- Preserve old prediction value
            return velocity
        end
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[getgenv().Forbidden.Target.Keybind:upper()] and PointLock.mode == "hold" then
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Forbidden.Silent.Enabled and IsTargeting then  
        UpdateFOV()  

        if getgenv().Forbidden.Silent.mode == "target" and TargetPlayer then
            if TargetPlayer.Character then
                ...
                if closestPart and closestPoint then
                    ...
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Forbidden.Silent.Prediction)
                end
            end
        elseif getgenv().Forbidden.Silent.mode == "normal" then
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().Forbidden.Silent.Enabled then
            Fov.Visible = IsTargeting and getgenv().Forbidden.Silent.AimSettings.FovSettings.FovVisible  
        end
    end
end)
    ...
    if tool:IsA("Tool") then
        tool.Activated:Connect(function()
            if tick() - lastToolUse > 0.1 then  
                ...
                local target
                if getgenv().Forbidden.Silent.mode == "target" then
                    target = TargetPlayer 
                elseif getgenv().Forbidden.Silent.mode == "normal" then
...
if getgenv().Forbidden.TargetControl.BlockGroundHits == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().Forbidden.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().Forbidden.Silent and getgenv().Forbidden.Silent)
                if TargetVelv5 then
